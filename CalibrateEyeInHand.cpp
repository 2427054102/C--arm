...
void str2Mat(vector<string> &strPosition, cv::Mat &toolPose) {
	...
	if (strPosition[0].size() == 30)
		...
		for (int i = 0; i < strPosition.size(); i++) {
			toolPose.at<double>(i, 0) = std::atoi(strPosition[i].substr(1, 7).c_str()) / 100.;
			toolPose.at<double>(i, 1) = std::atoi(strPosition[i].substr(8, 7).c_str()) / 100.;
			toolPose.at<double>(i, 2) = std::atoi(strPosition[i].substr(15, 7).c_str()) / 100.;
			toolPose.at<double>(i, 3) = std::atoi(strPosition[i].substr(22, 6).c_str()) / 100.;
			toolPose.at<double>(i, 4) = std::atoi(strPosition[i].substr(28, 6).c_str()) / 100.;
			toolPose.at<double>(i, 5) = std::atoi(strPosition[i].substr(34, 6).c_str()) / 100.;
			//cout << toolPose.at<double>(i, 0) << "	" << toolPose.at<double>(i, 1) << "	" << toolPose.at<double>(i, 2) << "	"
			//	<< toolPose.at<double>(i, 3) << "	" << toolPose.at<double>(i, 4) << "	" << toolPose.at<double>(i, 5) << endl;
		}
	//for (int j = 0; j < strPosition.size(); j++) {
	//	for (int i = 0; i < 3; i++) {
	//		string temp = strPosition[j].substr(1 + i * 7, 7);
	//		int val = atoi(temp.c_str());
	//		toolPose.at<double>(j, i) = val / 100.;
	//		//cout << "当前字串为：" << temp << "转换后数值为：" << v[i] << endl;
	//	}
	//	for (int i = 3; i < 6; i++) {
	//		string temp = strPosition[j].substr(22 + (i - 3) * 6, 6);
	//		int val = atoi(temp.c_str());
	//		toolPose.at<double>(j, i) = val / 100.;
	//		//cout << "当前字串为：" << temp << "转换后数值为：" << v[i] << endl;
	//	}
...
void str2Mat(const string strPosition, cv::Mat& toolPose) {
	...
	for (int i = 0; i < 3; i++) {
		string temp = strPosition.substr(1 + i * 7, 7);
		int val = atoi(temp.c_str());
		toolPose.at<double>(0, i) = val / 100.;
		//cout << "当前字串为：" << temp << "转换后数值为：" << v[i] << endl;
	}
	for (int i = 3; i < 6; i++) {
		string temp = strPosition.substr(22 + (i - 3) * 6, 6);
		int val = atoi(temp.c_str());
		toolPose.at<double>(0, i) = val / 100.;
		//cout << "当前字串为：" << temp << "转换后数值为：" << v[i] << endl;
	}
